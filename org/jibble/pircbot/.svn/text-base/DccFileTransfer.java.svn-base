...
public class DccFileTransfer {
    ...
    public String getTimeRemaining() {
        long seconds = (getSize()-getProgress())/getTransferRate();
        long hoursRemaining = TimeUnit.SECONDS.toHours(seconds);
        long minutesRemaining = TimeUnit.SECONDS.toMinutes(seconds) % 60;
        long secondsRemaining = seconds % 60;
        return "" + (hoursRemaining==0?"":hoursRemaining+" hrs ") + (minutesRemaining==0?"":minutesRemaining+" m ") + (secondsRemaining==0?"":secondsRemaining+" s ");
    }
}
